...
async def delete_recruitment_office(recruitment_office_id: int, db: AsyncSession = Depends(get_session)):
...
async def delete_mode_work(mode_work_id: int, db: AsyncSession = Depends(get_session)):
